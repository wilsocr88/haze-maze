		...
		for ( int x = 0; x < width; x = x + 1 ) {
			...
			for ( int y = drawStart; y < drawEnd; y++ ) {
				...
				if ( side == 0 )
					...
					color = ( textures.get( texNum ).pixels[texX + ( texY * textures.get( texNum ).SIZE )] >> 1 )
					        & 8355711;// Make y sides darker

				// Darken colors after a certain distance (fakie lighting)
				if ( lineHeight < 500 ) {
					float ratio = ( float ) ( lineHeight * 0.002 );
					int a = ( color >> 24 ) & 0xFF;
